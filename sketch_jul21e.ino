...
void loop() {
  if (recharge == false) { // Use false for recharge mode
    reCharge();
  } else {
    lcd.setCursor(0, 0);
    lcd.print("   Welcome!!!");
...
void rfid() {
  ...
  if (content.substring(1) == "23 4A 43 DD") {
    if (card1Balance >= 500) {
      lcdPrint();
  ...
  } else if (content.substring(1) == "83 71 D5 F8") {
    if (card2Balance >= 500) {
      lcdPrint();
  ...
  if (KState == PRESSED) {
    ...
    if (Key) {
      if (Key == 'A') {
        ...
        delay(1500);
        lcd.clear();
        recharge = false; // Set recharge mode to false
      }
    }
  }
}
...
void reCharge() {
  ...
  unsigned long lastInputTime = millis(); // Initialize last input time
  bool inputComplete = false; // Flag to check if input is complete
  ...
  while (!inputComplete) {
    ...
    if (KState == PRESSED) {
      ...
      if (Key) {
        if (Key == '#') {
          if (data_count > 0) { // Process the recharge amount only if data is entered
            processRecharge();
            inputComplete = true; // Mark input as complete
          } else {
            displayError("No amount entered.");
            clearData();
          }
        } else if (Key >= '0' && Key <= '9') {
          if (data_count < num - 1) { // Limit to 6 digits
            Data[data_count++] = Key;
            lcd.setCursor(data_count - 1, 1);
            lcd.print(Key);
            lastInputTime = millis(); // Reset the timer
          } else {
            displayError("Error: Max 6 digits");
          }
        } else {
    ...
    // Automatically process the input after 5 seconds of inactivity
    if (millis() - lastInputTime > 5000 && data_count > 0) {
      processRecharge();
      inputComplete = true; // Mark input as complete
    }
  }
...
void processRecharge() {
  if (card == "23 4A 43 DD") {
    ...
    displayBalance(card1Balance);
  } else if (card == "83 71 D5 F8") {
    num2 = Data;
    card2Balance = num2.toInt() + card2Balance;
    displayBalance(card2Balance);
  }
  clearData();
  recharge = true; // Exit recharge mode
}
